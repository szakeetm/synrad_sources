...
double NaN;
static const char *fileFilters[] = { "All files" , "*.*" , "Text files" , "*.txt" };
...
GLChart::~GLChart() {
  ...
  void GLChart::mousePressed(SDL_Event *evt) {
    ...
    if (evt->button.button == SDL_BUTTON_RIGHT) {
      ...
      int menuId = chartMenu->Track(GetWindow(), posX+ex, posY+ey);
      if( id>=0 ) {
        int nbV1 = y1Axis->GetViewNumber();
        int nbV2 = y2Axis->GetViewNumber();
  ...
  void GLChart::SaveFile() {
    ...
    FILENAME *fn = GLFileBox::SaveFile(lastDir,lastFile,"Save Data File",(char **)fileFilters,nbFilter);
    if( fn ) {
...
#define TRUNC(x) {int l = (int)strlen(x)-1; if(l>=0 && x[l]<32) x[l]=0; }
  ...
  void GLChart::LoadFile() {
    ...
    FILENAME *fn = GLFileBox::OpenFile(lastDir,lastFile,"Load Data File",(char **)fileFilters,nbFilter);
    if( fn ) {
