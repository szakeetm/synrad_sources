		...
		if (found) {
			...
			if (collidedFacet->sh.teleportDest) {
				...
				if (collidedFacet->sh.superDest) {	// Handle super structure link facet
					...
					if (collidedFacet->sh.reflectType < 2) { //Diffuse or mirror reflection
						DoReflection(sHandle->lowFluxMode, collidedFacet, collidedFacet->sh.sticking);
					}
					else { //material / rough surface reflection
						if (sHandle->dF == 0.0 || sHandle->dP == 0.0 || sHandle->energy < 1E-3) { //stick non real photons (from beam beginning)
							...
							if ((collidedFacet->sh.reflectType - 2) < (int)sHandle->materials.size()) { //found material type
								...
								do { //generate angles until reflecting away from surface
									...
									if (u < 0.0) phi = PI - phi;  // Angle to U
									double reflProbability = sHandle->materials[collidedFacet->sh.reflectType - 2].Interpolate(sHandle->energy, abs(theta - PI / 2));
									reflected = DoReflection(sHandle->lowFluxMode, collidedFacet, 1.0 - reflProbability, theta, phi, N_rotated, nU_rotated, nV_rotated);
...
BOOL DoReflection(BOOL lowFluxMode, FACET* collidedFacet, double stickingProbability, double theta, double phi, Vector N_rotated, Vector nU_rotated, Vector nV_rotated) {
	if (lowFluxMode) { //original algorithm, bounce or stick
		return DoLowFluxReflection(collidedFacet, stickingProbability, theta, phi, N_rotated, nU_rotated, nV_rotated);
	}
	else { //low flux mode								
		return DoRegularReflection(collidedFacet, stickingProbability, theta, phi, N_rotated, nU_rotated, nV_rotated);
	}
}

BOOL DoLowFluxReflection(FACET* collidedFacet, double stickingProbability, double theta, double phi, Vector N_rotated, Vector nU_rotated, Vector nV_rotated) {
...
BOOL DoRegularReflection(FACET* collidedFacet, double stickingProbability, double theta, double phi, Vector N_rotated, Vector nU_rotated, Vector nV_rotated) {
...
int PerformBounce_old(FACET *iFacet, double theta, double phi,
	Vector N_rotated, Vector nU_rotated, Vector nV_rotated) {
	...
	default: //Material reflection - theta, etc. are already calculated
		...
		if (!RoughReflection(iFacet, theta, phi,
