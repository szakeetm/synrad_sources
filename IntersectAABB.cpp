					...
					if (u >= 0.0 && u <= 1.0) {
						...
						if (v >= 0.0 && v <= 1.0) {
							...
							if (d > 0.0) {
								...
								if (IsInFacet(f, u, v)) {
									if ((f->sh.opacity < 0.999999 //Partially transparent facet
										&& rnd()>f->sh.opacity)
										...
										acos(DOT3(sHandle->pDir.x, sHandle->pDir.y, sHandle->pDir.z, f->sh.N.x, f->sh.N.y, f->sh.N.z)) - PI / 2, rnd()) == REFL_TRANS)) { //Transparent for this angle and energy
