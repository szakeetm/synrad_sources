...
DWORD Geometry::GetGeometrySize(std::vector<Region_full> *regions, std::vector<Material> *materials, std::vector<std::vector<double>> &psi_distr, std::vector<std::vector<double>> &chi_distr) {
	...
	//Regions
	memoryUsage += (int)(*regions).size()*sizeof(SHREGIONPARAM);
	for (int i=0;i<(int)(*regions).size();i++) {
		...
		memoryUsage += 6*sizeof(double)*(*regions)[i].beta_x_distr->size;
	}
	//Material library
...
void Geometry::CopyGeometryBuffer(BYTE *buffer, std::vector<Region_full> *regions, std::vector<Material> *materials,
	...
	for (int i=0;i<sh.nbRegion;i++) {
		...
		par->nbDistr_MAG=(*regions)[i].nbDistr_MAG;
		par->nbPointsToCopy=(*regions)[i].nbPointsToCopy;
		par->nbDistr_BXY = (*regions)[i].nbDistr_BXY;
		par->energy_low=(*regions)[i].energy_low;
		par->energy_hi=(*regions)[i].energy_hi;
		par->dL=(*regions)[i].dL;
		par->particleMass=(*regions)[i].particleMass;
		par->E=(*regions)[i].E;
		par->current=(*regions)[i].current;
		par->emittance=(*regions)[i].emittance;
		par->betax=(*regions)[i].betax;
		par->betay=(*regions)[i].betay;
		par->eta=(*regions)[i].eta;
		par->etaprime=(*regions)[i].etaprime;
		par->psimaxX=(*regions)[i].psimaxX;
		par->psimaxY=(*regions)[i].psimaxY;
		par->coupling=(*regions)[i].coupling;
		par->energy_spread=(*regions)[i].energy_spread;
		par->Bx_mode=(*regions)[i].Bx_mode;
		par->By_mode=(*regions)[i].By_mode;
		par->Bz_mode=(*regions)[i].Bz_mode;
		par->beta_kind=(*regions)[i].beta_kind; //switches between constant or file-based magnetic fields
		par->enable_ort_polarization=(*regions)[i].enable_ort_polarization;
		par->enable_par_polarization = (*regions)[i].enable_par_polarization;
		par->Bx_dir=(*regions)[i].Bx_dir;
		par->By_dir=(*regions)[i].By_dir;
		par->Bz_dir=(*regions)[i].Bz_dir; //direction in which MAG files are oriented (in their second line)
		par->Bx_period=(*regions)[i].Bx_period;
		par->By_period=(*regions)[i].By_period;
		par->Bz_period=(*regions)[i].Bz_period; //first line of length files
		par->Bx_phase=(*regions)[i].Bx_phase;
		par->By_phase=(*regions)[i].By_phase;
		par->Bz_phase=(*regions)[i].Bz_phase; //phase in case of helicoidal B file
		par->startPoint=(*regions)[i].startPoint;
		par->startDir=(*regions)[i].startDir;
		par->B_const=(*regions)[i].B_const;
		par->limits=(*regions)[i].limits;//AABBmin,AABBmax
		par->quad=(*regions)[i].quad;
		par->gamma=(*regions)[i].gamma;  //    =E/abs(particleMass)
